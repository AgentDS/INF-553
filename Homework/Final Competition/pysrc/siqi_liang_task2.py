...
def user_based_predict(pairs, user_ratings_bc, business_ratings_bc, user_bidxs_bc, business_uidx_bc, global_avg_bc, neighbor_K):
    for pair in pairs:
        user1 = pair[0]
        business = pair[1]
...
def user_based_CF(train_file_path, test_file_path, output_file_path, numPartitions):
    ...
    business_ratings = train_data.mapPartitions(
        lambda lines: emit_rating_pairs_user_base(uid2idx_bc, bid2idx_bc, key='business', method=1)).reduceByKey(
        lambda a, b: a + b).collectAsMap()
    ...
    test_pred_id_pairs = test_pairs.mapPartitions(
        lambda pairs: user_based_predict(pairs, user_ratings_bc, business_ratings_bc, user_bidxs_bc, business_uidx_bc, global_avg_bc,
                                         neighbor_K=2000000)).mapPartitions(
